...
import { isLoggedIn } from "../misc/common";
...
export async function BOOKING(obj) {
  try {
    if (localStorage.getItem("accessToken")) obj.accesstoken = localStorage.getItem("accessToken");
...
export async function RETRY_PAYMENT(voucherId) {
  if (isLoggedIn()) {
    try {
      let result = await axios.get("/v1/hotel/booking/payment-link?voucherId=" + voucherId);
      console.log(result);
      return result.data.response;
    } catch (error) {
      if (error.response) {
...
export async function CANCEL_BOOKING(voucherId) {
  Alert("Please wait!", "As per your request we are cancelling your hotel", "success", 80000);
  try {
...
export async function POPULER_HOTELS_DESTINATIONS() {
  try {
    let result = await axios.get("/v1/hotel/popular-hotels-destinations");
