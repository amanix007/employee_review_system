...
if (localStorage.getItem("accessToken")) {
  ...
  // localStorage.getItem('accessToken');
  // axios.defaults.headers.put["accessToken"] =
  // localStorage.getItem('accessToken');
  // axios.defaults.headers.patch["accessToken"] =
  // localStorage.getItem('accessToken');
...
export async function FLIGHT_BOOKING_HISTORY() {
  try {
    // let result = await axios.get("/v1/flight/booking/history?accessToken=" +
    // data);
...
export async function GATEWAY(serviceType, gatewayCurrency, bankCode) {
  try {
    let string = `/v1/payment/gateway?service=${serviceType}`;
    if (gatewayCurrency) {
      string = string + `&currency=${gatewayCurrency}`;
    }
    if (bankCode) {
      if (bankCode.length > 0) {
        let bankCodeString = "";
        for (let i = 0; i < bankCode.length; i++) {
          bankCodeString = bankCodeString + `&bankCode[]=${bankCode[i]}`
        }
        string = string + bankCodeString;
      }
    }
...
export async function TRACK_FLIGHT(airline, flightnum, date) {
  try {
    let result = await axios.get(`v1/flight/tracker/?flight=${airline}${flightnum}&date=${date}`);
    if (!result.data.error) {
      return result.data.response;
    }
  } catch (error) {
...
export async function GET_TOP_AIRLINE() {
  try {
    let result = await axios.get("/v1/flight/top-airlines/");
...
export async function GET_COUNTRYWISE_AIRLINE(countryCode) {
  try {
    let result = await axios.get("/v1/flight/airlines-country?country=" + countryCode);
...
export async function GET_AIRLINE_DETAILS(name) {
  try {
    let result = await axios.get("/v1/flight/airlines/?slag=" + name);
...
export async function GET_CHEAP_ROUTES() {
  try {
    let result = await axios.get("/v1/flight/cheapest-flight");
...
export async function GET_LUGGAGE(searchString) {
  try {
    // let result = await axios.get(`/v1/flight/luggage?searchId=${searchId}&sessionId=${sessionId}&sequenceCode=${sequenceCode}`);
    let result = await axios.get(`/v1/flight/luggage${searchString}`);
...
export async function GET_COVID_TEST_OPTIONS() {
  try {
    let result = await axios.get(`/v1/covid/addon-service`);
    if (!result.data.error) {
      return result.data.response;
    }
    Alert("Oops!", result.data.message, "error");
...
export async function GET_COVID_TEST_INFO(code) {
  try {
    let result = await axios.get(`/v1/covid/service?code=${code}`);
    if (!result.data.error) {
      return result.data.response;
    }
    Alert("Oops!", result.data.message, "error");
